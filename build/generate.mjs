...
<script>window.V4_TRACK={META_PIXEL_ID:'',GA4_ID:''};</script>
...
<style>
  ...
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
  ...
  .lead-form{display:flex;flex-direction:column;gap:.55rem;margin-top:1.6rem;max-width:440px;border-top:1px solid var(--blue-100);padding-top:1.4rem}
  .lead-form h3{font-size:.95rem;font-weight:600;color:var(--blue-900);margin-bottom:.15rem}
  ...
  <div class="top">
    ...
    <div>
      ...
      <form class="lead-form" id="leadForm">
        <h3>Tenho interesse nesta aeronave</h3>
        <div class="lf-row">
        ...
        <button type="submit" class="btn btn-p" id="lf_btn">Falar sobre esta aeronave</button>
...
(function(){
  ...
  var note=document.getElementById('lf_note');
  ...
  f.addEventListener('submit',function(ev){
    ...
    fetch(SB+'/rest/v1/leads',{method:'POST',headers:{'Content-Type':'application/json',apikey:KEY,Authorization:'Bearer '+KEY,Prefer:'return=minimal'},body:JSON.stringify({nome:nome,telefone:tel,email:email||null,aeronave:AERO,origem:'aeronave:'+AERO})}).catch(function(){});
    track();
    var txt='Olá, sou '+nome+'. Tenho interesse na aeronave '+AERO+' anunciada no site.';
    window.open('https://wa.me/'+WA+'?text='+encodeURIComponent(txt),'_blank','noopener');
    note.hidden=false;note.className='lf-note ok';note.textContent='Recebido! Abrindo o WhatsApp…';
    f.reset();
  });
})();
</script>
